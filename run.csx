...
public static async Task Run(EventData[] events, ILogger log)
{
    ...
    foreach (EventData eventData in events)
    {
        try
        {
            ...
            var insertScript = $"INSERT INTO [dbo].[Telemetry] ([Building],[Parameter],[Value],[TimeStamp]) VALUES (@Building, @Parameter, @Value, @Date)";
            using (SqlConnection connection = new SqlConnection(cs))
            {
                SqlCommand command = new SqlCommand(insertScript, connection);
                command.Parameters.AddWithValue("@Building", "4630");
